...
class CPlotItem {
public:
  CPlotItem():hist1F(0),hist1D(0),hist2D(0),graph(0),prof(0),drawopt(""){}
  ~CPlotItem(){}
...
//  void Clear(){}
  ...
  // Add a latex string
  void AddLatex(TLatex *latex) {
    assert(latex);
    fLatex.push_back(latex);
  }
  ...
  TString GetXTitle() const { return fXTitle; }
  void SetYTitle(TString str)              { fYTitle = str; }              // y-axis title
  TString GetYTitle() const { return fYTitle; }
...
protected:
  ...
  vector<TLatex*> fLatex;        // list of latex strings
...
public:
  int LegendIsNonNull() const { return (fLeg)? 1:0; }
};
