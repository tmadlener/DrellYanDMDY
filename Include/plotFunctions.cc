...
//acceptance, bkgRatesPercent, LEGO2,COLZ 
   ...
   TH2D* Hist= new TH2D(histName,title, nM, DYTools::massBinLimits, nY, 
			DYTools::XXRangeMin, DYTools::XXRangeMax);
   Hist->SetDirectory(0);
   for (int i=0; i<nM; i++)
     for (int j=0; j<nY; j++)
       {
...
TMatrixD AdjustMatrixBinning(const TMatrixD &matrUsualBinning)
{
  ...
  for (int i=0; i<DYTools::nMassBins; i++)
    {
      int nTheSameCells=minMutualMultiple()/DYTools::nXXBins[i];
      for (int j=0; j<DYTools::nXXBins[i]; j++)
...
{
  int mult=DYTools::nXXBins[0];
  for (int i=1; i<DYTools::nMassBins; i++)
  {
    mult=minMutualMultipleTwo(mult,DYTools::nXXBins[i]);
...
TMatrixD relPostFsrCrossSectionDET, TMatrixD relPostFsrCrossSectionStatErrDET)
{
   ...
   if (DYTools::study2D==0)
     ...
     {
       x  = new double[DYTools::nXXBinsMax];
       ex = new double[DYTools::nXXBinsMax];       
       y1  = new double[DYTools::nXXBinsMax];
       ey1 = new double[DYTools::nXXBinsMax];
       y2  = new double[DYTools::nXXBinsMax];
       ey2 = new double[DYTools::nXXBinsMax];
       y3  = new double[DYTools::nXXBinsMax];
       ey3 = new double[DYTools::nXXBinsMax];
       y4  = new double[DYTools::nXXBinsMax];
       ey4 = new double[DYTools::nXXBinsMax];
        ...
        for (int i=0; i<DYTools::nMassBins; i++)
         {
           ...
           for (int j=0; j<DYTools::nXXBins[i]; j++)
             {
               x[j]=DYTools::XXRangeMin+
		 (j+0.5)*(DYTools::XXRangeMax-DYTools::XXRangeMin)/n;
               ex[j]=0.5*(DYTools::XXRangeMax-DYTools::XXRangeMin)/n;
 ...
 //
  ...
  for(UInt_t isam=0; isam < samplev.size(); isam++){
    ...
    for(int im = 0; im < DYTools::nMassBins; im++){
      for(int iy = 0; iy < DYTools::nXXBins[im]; iy++){
		   ...
		   TFile *histoFile) 
{
  ...
  // loop over mass bins starting from 1st (excluding underflow bin)
    {
      ...
      totalMcName+=i;
      totalMc[i]=new TH1F(totalMcName,"",DYTools::nXXBins[i],DYTools::XXRangeMin,DYTools::XXRangeMax); 
      TString ewkName="ewk-" + normStr;
      ewkName+=i;
      ewkHist[i]=new TH1F(ewkName,"",DYTools::nXXBins[i],DYTools::XXRangeMin,DYTools::XXRangeMax);
      ...
      // loop over data, signal MC and background MC samples
        {
          ...
          allHists[i][j]=new TH1F(histName,"",DYTools::nXXBins[i],DYTools::XXRangeMin,DYTools::XXRangeMax);
          //nXXBins, XXRangeMin, XXRangeMax - from DYTools.hh

          TMatrixD *thisSampleYields = yields.at(j);
          TMatrixD *thisSampleYieldsSumw2 = yieldsSumw2.at(j);

          for (int k=0; k<DYTools::nXXBins[i]; k++)
  ...
  for (int i=1; i<DYTools::nMassBins; i++) {
	...
	XXMax = 1.5;
      ...
      ratioHist[i]->GetYaxis()->SetRangeUser(XXMin,XXMax);
			...
			double yAxisMin, double yAxisMax)
{
  ...
  if (nY!=DYTools::nXXBinsMax) {
    std::cout << "PlotMatrixMYSlices: nN!=DYTools::nXXBinsMax" << std::endl;
    throw 2;
  }
  for (unsigned int i=0; i<matrV.size(); ++i) {
    ...
    if (nY!=matrV[i].GetNcols()) {
      std::cout << "PlotMatrixMYSlices: matrV[" << i << "].GetNcols!=DYTools::nXXBinsMax" << std::endl;
      throw 2;
    }
  }
  for (unsigned int i=0; i<matrErrV.size(); ++i) {
    ...
    if (nY!=matrErrV[i].GetNcols()) {
      std::cout << "PlotMatrixMYSlices: matrErrV[" << i << "].GetNcols!=DYTools::nXXBinsMax" << std::endl;
      throw 2;
    }
  }
  ...
  if (!functionOfRapidity) {
    for (unsigned int ii=0; ii<indices.size(); ++ii) {
      ...
      DYTools::findAbsXXValueRange(0, iY, XXMin, XXMax);
      double yCenter=0.5*(XXMin+XXMax);
	...
	for (int i=0; i<nM; i++) {
	  int iRap=DYTools::findAbsXXBin(i, yCenter);
	...
	snprintf(histBuf, hBufLen,"h%d_%s_mass%1.0f_%1.0f",iSet,histName.Data(),
		 DYTools::massBinLimits[iMass], DYTools::massBinLimits[iMass+1]);
	TH1F* Hist= new TH1F(histName,"", DYTools::nXXBins[iMass], DYTools::XXRangeMin, DYTools::XXRangeMax);
	Hist->SetDirectory(0);
	//Hist->GetXaxis()->SetTitle("|Y|");
	Hist->SetMarkerColor(colors[iSet%colorCount]);
	for (int j=0; j<DYTools::nXXBins[iMass]; j++) {
	  if (printData) {
	    double XXMin,XXMax;
	    DYTools::findAbsXXValueRange(0, j, XXMin,XXMax);
	    printf(" %6.1f-%6.1f  %8.2lf  %8.2lf\n",XXMin,XXMax,matrV[iSet](iMass,j),matrErrV[iSet](iMass,j));
	  }
...
void PlotMatrixMYSlices(const std::vector<int> &indices, int functionOfRapidity,
  ...
  for (unsigned int i=0; res && (i<matrFIV.size()); ++i) {
    TMatrixD temp(DYTools::nMassBins,DYTools::nXXBinsMax);
			 ...
			 const std::vector<TString> &labelV)
{
  ...
  int nY=matrV[0].GetNcols();
  if (nY!=DYTools::nXXBinsMax) {
    std::cout << "PrintMatrixMYSlices: nN!=DYTools::nXXBinsMax" << std::endl;
    throw 2;
  }
  for (unsigned int i=0; i<matrV.size(); ++i) {
    if (nM!=matrV[i].GetNrows()) {
    ...
    if (nY!=matrV[i].GetNcols()) {
      std::cout << "PrintMatrixMYSlices: matrV[" << i << "].GetNcols!=DYTools::nXXBinsMax" << std::endl;
      throw 2;
    }
  }
  for (unsigned int i=0; i<matrErrV.size(); ++i) {
    ...
    if (nY!=matrErrV[i].GetNcols()) {
      std::cout << "PrintMatrixMYSlices: matrErrV[" << i << "].GetNcols!=DYTools::nXXBinsMax" << std::endl;
      throw 2;
    }
  }
