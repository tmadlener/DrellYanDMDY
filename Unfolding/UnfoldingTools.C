...
// -------------------------------------------------------
  ...
  void writeBinningArrays(TFile &fout) {
    ...
    for (int i=0; i<nMassBins  ; i++) rapidityCounts[i]=nXXBins[i];
  ...
  bool checkBinningRanges(const TVectorD &mass, const TVectorD &rapidityCounts, const TString &fname) {
	...
	if ( nXXBins[i] != rapidityCounts[i] ) {
	  std::cout << fncname 
		    << "y bin count " << fileInfo
		    << " at i=" << i 
		    << " is " << rapidityCounts[i] 
		    << " instead of expected " << nXXBins[i] << "\n";
	  rapidityOk=false;
	}
      }
    }
